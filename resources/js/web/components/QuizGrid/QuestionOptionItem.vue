<template>
    <div>
        <button
            ...
            :class="{
                    'bg-slate-50 outline outline-1 outline-gray-300 hover:outline-gray-600 text-slate-800': !question?.answered && !option?.selected && !store.evaluated,
                    ...
                    'bg-green-100  outline outline-1 outline-gray-300 text-slate-600': store.evaluated && !option?.selected && option?.evaluation?.rightAnswer,
                    'bg-rose-600 text-white': store.evaluated && option?.selected && !option?.evaluation?.rightAnswer,
                    'bg-red-100  outline outline-1 outline-gray-300 text-slate-600': store.evaluated && !option?.selected && !option?.evaluation?.rightAnswer,
