<template>
    <div class="container mx-auto px-4 py-6">
        <div v-if="selectedIsland" class="relative mx-auto mb-3 flex max-w-5xl items-center justify-center sm:hidden">
            <button
                type="button"
                class="absolute left-0 z-10 inline-flex items-center gap-2 rounded-lg bg-white/95 px-2.5 py-[0.3rem] text-[0.95rem] font-semibold leading-tight text-blue-900 shadow-md transition hover:bg-white"
                @click="close"
                aria-label="Zpět"
            >
            ...
            <h2 class="island-detail-label">
                {{ selectedIsland.name }}
                <span class="island-progress island-progress--lg">{{ selectedIsland.tasks.completed }}/{{ selectedIsland.tasks.total }}</span>
            </h2>
        </div>

        <div class="island-scene relative mx-auto aspect-[3/2] w-full max-w-5xl overflow-hidden rounded-2xl shadow-lg">
            ...
            <transition name="scene-fade">
                <div v-if="!selectedIsland" key="scene" class="absolute inset-0">
                    ...
                    <div class="island-wrap absolute top-1/2 left-1/2 w-1/3 -translate-x-1/2 -translate-y-1/2">
                    ...
                    <button
                        ...
                        :key="island.key"
                        type="button"
                        class="island-wrap absolute w-1/5 cursor-pointer border-0 bg-transparent p-0 transition-transform hover:scale-105 focus:scale-105 focus:outline-none"
                ...
                <div v-else key="detail" class="absolute inset-0 flex flex-col p-3 sm:p-6">
                    <div class="relative mb-4 hidden items-center justify-center sm:flex">
                        <button
                            type="button"
                            class="absolute left-0 z-10 inline-flex items-center gap-2 rounded-lg bg-white/95 px-2.5 py-[0.3rem] text-[0.95rem] font-semibold leading-tight text-blue-900 shadow-md transition hover:bg-white sm:px-4 sm:py-2 sm:text-base"
                            @click="close"
                            aria-label="Zpět"
                        >
                            <span aria-hidden="true">←</span>
...
const islandBoxes = [
    { top: 30, left: 33, bottom: 70, right: 67 },  // maják
    { top: 14, left: 10, bottom: 34, right: 34 },  // top-left
    { top: 14, left: 66, bottom: 34, right: 90 },  // top-right
    { top: 66, left: 10, bottom: 86, right: 34 },  // bottom-left
    { top: 66, left: 66, bottom: 86, right: 90 },  // bottom-right
];

// scéna 3:2 (W:H) → SVG viewBox 40:8 means height = width * 0.2 v poměru šířky kontejneru,
// tj. v % výšky kontejneru je vlna vysoká width * 0.2 * 1.5 = width * 0.3.
...
const generateWavelets = (count) => {
    ...
    while (placed.length < count && safety < count * 80) {
        ...
        const left = 2 + Math.random() * (96 - width);
        const top = 2 + Math.random() * (96 - height);
        const candidate = { top, left, width, height };

        const onIsland = islandBoxes.some(b => boxesOverlap(candidate, {
            top: b.top,
            left: b.left,
...
const seaWavelets = generateWavelets(45);
...
const lighthouseImg = asset('lighthouse.png');

const islands = reactive([
    {
        key: 'digitalni-pasti',
        name: 'Ostrov digitálních pastí',
        image: asset('digital_traps.png'),
        detailImage: asset('detail/digital_traps.png'),
    ...
    },
    {
        ...
        image: asset('deceptive_news.png'),
        detailImage: asset('detail/deceptive_news.png'),
    ...
    },
    {
        ...
        image: asset('exploited_emotions.png'),
        detailImage: asset('detail/exploited_emotions.png'),
    ...
    },
    {
        ...
        image: asset('fake_wealth.png'),
        detailImage: asset('detail/fake_wealth.png'),
...
<style scoped>
.island-scene {
    background: radial-gradient(ellipse at 50% 25%, #4ea6d8 0%, #2c79b0 55%, #144a78 100%);
}
